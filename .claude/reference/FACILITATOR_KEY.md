...
This file lives under `.claude/reference/`, behind `reference_guard.py`. A learner session cannot
read it, and cannot read the pointer stub at `boost-authentication-service/docs/FACILITATOR_KEY.md`
either. To open it yourself: `export WORKBENCH_FACILITATOR=1`, read, then `unset` it.
...
**Deterministic backstop.** Either way, the negative test proves the point: RED on the seeded
draft (`authenticatePayer` invoked once), GREEN once corrected. The teaching moment never depends
on one probabilistic pass.

`NoSecondAuthenticatePayerCallTest` is the house name for that test, and the participant guide
asks for it at Stage 5 — but grading finds the test by its **assertion**
(`never()` … `authenticatePayer`), not by its filename, so a group that named it
`ReadOnlyRetrievalTest` is graded exactly the same. If you are checking by hand, look for the
...
## 5a. Stage 6 — making the PR gate fire on cue

Do not rely on a gate happening to be red. If a group finished Stages 4 and 5 cleanly, every gate
is green and the guard will simply allow the write — the control never fires and the point is
lost.

The scripted version (in the participant guide): the learner adds one line to
`PayerAuthenticationService` — `log.info("retrieved {}", stored);` — attempts the PR write, and is
denied because the sensitive-data scan now finds a PAN and a CAVV in `logs/auth-service.log`. Then
they delete the line and re-run.

It lands two things at once: the control is structural rather than advisory, and the leak that
tripped it was a single line of well-meant debug logging written by someone who knew the rule.
A group that still has a genuinely red gate does not need the setup — let theirs fire.

The guard also covers `Bash` writes (`>`, `tee`, `cp`, `sed -i`, `open(..., "w")`), so "just write
it with a heredoc" is not a way past it. Reading the file is still allowed.
...
| plugin PAN gate never blocks | it detects the PAN and exits 1; only exit 2 (or a JSON deny) blocks a tool call | Same mitigation — `pan_gate.py` returns a proper deny. Do not tell the room the plugin gate blocks; the lab-local one does. |
| journey files scatter | the hook writes `journey/` relative to the process working directory, so events also land in `<repo>/journey/` | Expected. `/hand-off` records boundaries into the workspace-root journey wherever it is run from, and the grader reads every `journey/` in the workspace. |
